...
interface ReporterOptions {
  ...
  webhookUrl?: string;
  outputDir?: string;
  liveServerPort?: number; // NEW: Port for live reporting
}
...
class PlayWithAIReporter implements Reporter {
  private verbose: boolean;
  private slowTestThreshold: number;
  private logFile: string;
  private webhookUrl?: string;
  private outputDir: string;
  private liveServerPort: number;
  ...
  private startTime = 0;
  private totalTestsExpected = 0;

  private slowTests: { title: string; duration: number; browser: string }[] = [];
  private failedTests: { title: string; browser: string }[] = [];
  private allTestRecords: TestRecord[] = [];
  ...
  private notifyClients(newRecord: TestRecord | null) {
  ...
  async onTestEnd(test: TestCase, result: TestResult) {
    ...
    let newRecord: TestRecord | null = null;
    ...
    if (!isIntermediateFailure) {
        newRecord = {
            ...
            title: test.title,
            project: browser,
            status: (consoleStatus || result.status) as any,
  ...
  private generateMarkdown(duration: string, total: number) { /* ... same as before ... */ }
  private async sendWebhook(status: string, duration: string) { /* ... same as before ... */ }

  // --- UPDATED HTML TEMPLATE FOR LIVE UPDATES ---
  private getHtmlTemplate(stats: Stats, tests: TestRecord[], isLive: boolean): string {
